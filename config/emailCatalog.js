...
const TEMPLATES = [
  ...
  {
    key: "crm.taskAssigned",
    label: "Task assigned (team alert)",
    group: "sales",
    scope: "platform",
    audience: "Platform team",
    description: "Tells an operator that a CRM follow-up task has been put on their plate.",
    variables: [
      ...RECIPIENT_VARS,
      v("task.title", "Task", "Call Riverbank Care back about pricing"),
      v("task.type", "Type", "call"),
      v("task.priority", "Priority", "high"),
      v("task.dueAt", "Due", "2026-09-04T04:00:00.000Z"),
      v("task.url", "Task link", "https://admin.donexus.app/tasks/123"),
      v("lead.orgName", "Related lead", "Riverbank Care"),
      v("staff.name", "Assigned by", "Aisha Khan"),
    ],
    defaults: {
      subject: "Task assigned: {{task.title}}",
      preheader: "{{staff.name | default:\"Someone\"}} assigned you a follow-up",
      blocks: [
        { ...h("A task is waiting on you"), eyebrow: "{{task.priority | default:\"normal\"}} priority" },
        p("Hi {{recipient.firstName | default:\"there\"}}, {{staff.name | default:\"someone on the team\"}} assigned you this."),
        panel("The task", [
          { label: "Task", value: "{{task.title}}", strong: true },
          { label: "Type", value: "{{task.type}}", showIf: "task.type" },
          { label: "Due", value: "{{task.dueAt | datetime}}", showIf: "task.dueAt" },
          { label: "Lead", value: "{{lead.orgName}}", showIf: "lead.orgName" },
        ]),
        btn("Open the task", "{{task.url}}"),
      ],
    },
  },
  {
    key: "crm.taskDigest",
    label: "Daily task digest (team)",
    group: "sales",
    scope: "platform",
    audience: "Platform team",
    description: "The morning list of an operator's overdue and due-today CRM tasks. Sent only when they have some.",
    variables: [
      ...RECIPIENT_VARS,
      v("tasks.overdueCount", "Overdue count", 2),
      v("tasks.todayCount", "Due-today count", 3),
      v("tasks.items", "Tasks (list)", [
        { title: "Call Riverbank Care back about pricing", due: "Overdue by 2 days", lead: "Riverbank Care" },
        { title: "Send the Professional proposal", due: "Today, 5:00 pm", lead: "Harbour Youth" },
      ]),
      v("tasks.url", "Task list link", "https://admin.donexus.app/tasks?due=today"),
    ],
    defaults: {
      subject: "{{tasks.overdueCount}} overdue, {{tasks.todayCount}} due today",
      preheader: "Your follow-ups for today",
      blocks: [
        { ...h("Today's follow-ups"), eyebrow: "Sales" },
        p("Morning {{recipient.firstName | default:\"there\"}} — here's what's on your plate."),
        table("tasks.items", [
          { label: "Task", value: "{{this.title}}" },
          { label: "Lead", value: "{{this.lead}}" },
          { label: "Due", value: "{{this.due}}", align: "right" },
        ]),
        btn("Open my tasks", "{{tasks.url}}"),
      ],
    },
  },
  {
